import numpy as np
def find_curvatures(waypoints):
    n = len(waypoints)
    for i in range(1,n-1):
        p,q,r = waypoints[i],waypoints[i-1],waypoints[i+1]
        x1,y1,x2,y2,x3,y3 = p.x,p.y,q.x,q.y,r.x,r.y
        k1 = 0.5*((x1**2)+(y1**2)-(x2**2)-(y2**2))/(x1-x2+1e-9)
        k2 = (y1-y2)/(x1-x2+1e-9)
        print(x3,k2,y3,y2,x2,k2)
        b = 0.5*((x2**2)-(2*x2*k1)+(y2**2)-(x3**2)+(2*x3*k1)-(y3**2))/((x3*k2)-y3+y2-(x2*k2)+1e-9)
        a = k1-(k2*b)
        r = np.sqrt(((x1-a)**2)+((y1-b)**2))
        curvature = 1/r
        print("For point i", i ,"curvature is", 1/r)
...
def main():
    ...
    g = [42, 28, np.deg2rad(0)]

    # Get Obstacle Map
    obstacleX, obstacleY = map()

    # Calculate map Paramaters
    mapParameters = calculateMapParameters(obstacleX, obstacleY, 1, np.deg2rad(30.0))
