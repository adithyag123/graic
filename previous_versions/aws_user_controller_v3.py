    ...
    def get_ref_state(self, currState, obstacleList, lane_marker, waypoint):
        ...
        if obstacleList and self.target_lane == self.lane_state:
            for obs in obstacleList:
                ...
                rx = np.cos(-yaw) * dx - np.sin(-yaw) * dy
                x = np.sqrt(dy * dy + dx * dx)
                print("X, detect_dist, rx", x, self.detect_dist, rx)
        ...
        if obstacleList and len(obstacleList) < 3:
            self.velocity_multiplier = min(0.75, self.velocity_multiplier)
        ...
        if self.vehicle_state == "stop":
            ...
            self.speed = 30
        if front_dist < self.detect_dist/2 and pedestrian_dist < self.detect_dist:
            self.velocity_multiplier = min(0.5, self.velocity_multiplier)
        if front_dist < self.detect_dist/2 and pedestrian_dist < self.detect_dist/2:
            self.velocity_multiplier = min(0.025, self.velocity_multiplier)
        ...
        if ((distToTargetX < 5 and distToTargetY < 5)):
            ...
            self.counter += 1
        print("Targets", self.target_x, self.target_y)
